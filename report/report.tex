...
%
ciascuno dei quali conterrà stanze collegate tra loro. Ogni stanza potrà contenere una trappola, un nemico, un tesoro oppure ospitare il giocatore.

\begin{itemize}
    \item \textbf{Trappole}: infliggono danni al giocatore.
    \item \textbf{Nemici}: attaccano il giocatore in combattimenti a turni.
    \item \textbf{Tesori}: forniscono punti vita, esperienza o nuove armi.
\end{itemize}
%
...
%
Il giocatore e i nemici avranno a disposizione diverse tipologie di mosse di attacco, che possono variare in potenza ed effetto.
%
...
%
Durante la partita, sia il giocatore sia i nemici avranno due barre di stato: \textbf{vita} ed \textbf{esperienza}.
%
\begin{itemize}
    ...
    \item \textbf{Barra dell’esperienza}: aumenta con i tesori o combattendo nemici. Raggiunto un certo livello, il giocatore sbloccherà abilità o miglioramenti.
\end{itemize}
%
...
%
Per arricchire l’esperienza, sarà presente un sottofondo musicale durante tutto il gioco, accompagnato da effetti sonori che reagiscono alle azioni del giocatore, 
%
come combattimenti, vittorie o l'apertura di tesori.
%
\begin{figure}[H]
	\includegraphics[scale=0.15]{img/analysis.png}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{View}: 
    \begin{itemize}
        \item La classe \texttt{GameView} gestisce la presentazione grafica del gioco. 
        \item È responsabile della visualizzazione dello stato attuale della stanza (\texttt{displayRoom}), degli aggiornamenti della barra di stato del giocatore (\texttt{updateStatusBar}) e della gestione di eventi speciali come la vittoria o il game over.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Gestisce le azioni dell'utente (\texttt{handleAction}), permette al giocatore di muoversi tra le stanze (\texttt{changeRoom}) o salire al piano successivo (\texttt{gotoNextFloor}), e si occupa di iniziare e terminare il gioco.
    \end{itemize}
\end{itemize}
